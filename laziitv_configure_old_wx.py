...
class FlaskThread(multiprocessing.Process):

    def __init__(self, results, ppid):
        multiprocessing.Process.__init__(self)
        self.results = results
        self.ppid = ppid

    def run(self):
        #thread.start_new_thread(self.check_parent_id, (self.ppid,))
        flask_server.start_server(self.results)

    def check_parent_id(self, ppid):
        while True:
            time.sleep(0.25)
            print(ppid)
            try:
                os.kill(ppid, signal.SIG_IGN)
                print(ppid)
            except OSError as err:
                if err.errno == errno.ESRCH:
                    # ESRCH == No such process
                    print("EXXXXXXITTTTTTTTTT: " + str(ppid))
                    #sys.exit(1)
...
if __name__ == '__main__':
    print("CONFFFFFFFFFF: " + str(os.getpid()))
    #results = multiprocessing.Queue()
    #f = FlaskThread(results, os.getpid())
    #f.start()
